...
public class Backend implements BackendInterface {
  ...
  public static void main(String[] args) {
    ...
    // {"C:\\\\Users\\\\zxcvt\\\\Documents\\\\CS400\\\\MovieMapper\\\\movies.csv", "Horror", "2",
    // "0"};
    // Backend tester1 = new Backend(cmdArgs);
    try {
      Backend tester2 = new Backend(
          new FileReader("C:\\Users\\zxcvt\\Documents\\CS400\\MovieMapper\\movies.csv"));
  ...
  public void addGenre(String genre) {
    ...
    if (this.genre.contains(genre) == false) {
        ...
        if (pairs[i] != null) {
          ...
          while (currList != null) {
            ...
            for (int j = 0; j < tempList.size(); j++) {
              ...
              if (currNode.getGenres().containsAll(this.genre) == true
                  && this.possibleGenres.contains(currNode) == false) {
      ...
      /*
       * if(this.rating.size() == 0) { for(int i = 0; i < pairs.length;i++) { if(pairs[i]!= null) {
       * PairNodeList<String, List<Movie>> currList = pairs[i]; while(currList != null) {
       * List<Movie> tempList = pairs[i].getValue(); for(int j = 0; j < tempList.size(); j++) {
       * Movie currNode = tempList.get(j); if(currNode.getGenres().containsAll(this.genre) == true
       * && this.possibleGenres.contains(currNode) == false) { this.possibleGenres.add(currNode); }
       * } currList = currList.getNext(); } } } } else { for(int i = 0; i < pairs.length;i++) {
       * if(pairs[i]!= null) { PairNodeList<String, List<Movie>> currList = pairs[i]; while(currList
       * != null) { List<Movie> tempList = pairs[i].getValue(); for(int j = 0; j < tempList.size();
       * j++) { Movie currNode = tempList.get(j); if(currNode.getGenres().containsAll(this.genre) &&
       * this.possibleGenres.contains(currNode) == false && checkRange(currNode) == true) {
       * this.possibleGenres.add(currNode); } } currList = currList.getNext(); } } } }
       */
  ...
  // This sorts through all the possible genres set and finds the ratings that are within the range
  // of rating + 1
      ...
      if (this.possibleMovies.size() > 0) {
        for (String rat : this.rating) {
          for (MovieInterface dw : this.possibleMovies) {
            ...
            if ((dw.getAvgVote() >= Float.parseFloat(rat)) && (dw.getAvgVote() < upperLim)
                && this.validMovies.contains(dw) == false) {
  ...
  // Removing the rating added in the selection criteria, all the movies that meet the criteria for
  // removal
  ...
  public void removeAvgRating(String rating) {
    ...
    for (int i = 0; i < this.possibleMovies.size(); i++) {
      ...
      if (this.possibleMovies.get(i).getAvgVote() >= tempRating
          && this.possibleMovies.get(i).getAvgVote() < upperLimit) {
  ...
  // Method that calls removeAll by comparing the genre passed in to those in possibleGenres
  // (ArrayList) and
    ...
    if (this.validMovies.size() > 0) {
      if ((startingIndex + 3) < this.validMovies.size()) {
        for (int i = startingIndex; i < startingIndex + 3; i++) {
          if (!((startingIndex + 3) > this.validMovies.size())
              && checkGenre(this.validMovies.get(i)) == true) {
      ...
      if ((startingIndex + 3) < this.possibleMovies.size()) {
        for (int i = startingIndex; i < startingIndex + 3; i++) {
          if (!((startingIndex + 3) > this.possibleMovies.size())
              && checkGenre(this.possibleMovies.get(i)) == true) {
      ...
      if ((startingIndex + 3) < this.possibleMovies.size()) {
        ...
        for (int j = startingIndex; j < this.possibleMovies.size(); j++) {
          if (checkGenre(this.possibleMovies.get(j)) == true
              && checkRange(this.possibleMovies.get(j)) == true) {
            this.outputList.add(this.possibleMovies.get(j));
          }
        }
      }
  ...
  // Main use is for adding to the possibleMovie list, which is the ArrayList that contains all the
  // movies
